...
twitch_miner = TwitchChannelPointsMiner(
    username="kirill_safe",
    password="kirill655",           # If no password will be provided, the script will ask interactively
    ...
    logger_settings=LoggerSettings(
        ...
        time_zone="Asia/Yekaterinburg",                           # Set a specific time zone for console and file loggers. Use tz database names. Example: "America/Denver"
        ...
        telegram=Telegram(                                                          # You can omit or set to None if you don't want to receive updates on Telegram
            chat_id=1991980261,                                                      # Chat ID to send messages @getmyid_bot
            token="7467184282:AAEUssoxeyynqwGEydXShSq32PEDFNvUprs",                          # Telegram API token @BotFather
            events=[Events.STREAMER_ONLINE, Events.STREAMER_OFFLINE, Events.GAIN_FOR_RAID, Events.DROP_STATUS, Events.DROP_CLAIM,
                    Events.BET_LOSE, Events.CHAT_MENTION],                                  # Only these events will be sent to the chat  # Only these events will be sent to the chat
            disable_notification=True,                                              # Revoke the notification (sound/vibration)
        ),
        discord=Discord(
            webhook_api="https://ptb.discord.com/api/webhooks/1272664085133262848/ZIjlErvv--mMr_IGzERAfBV0OIw3M8cOmd6F5AZhCtckPheqUy4f3--k_ZCL_ArUr5mJ",  # Discord Webhook URL
            events=[Events.STREAMER_ONLINE, Events.STREAMER_OFFLINE, Events.GAIN_FOR_RAID, Events.GAIN_FOR_CLAIM, Events.GAIN_FOR_WATCH, Events.BONUS_CLAIM, Events.DROP_STATUS, Events.DROP_CLAIM,
        ...
        pushover=Pushover(
            userkey="u1pmqpgc7ni3esoiefqdcxezjrn6uk",                                             # Login to https://pushover.net/, the user token is on the main page
            token="asxszcmtyv895agbtgxoyeatw49xdt",                                           # Create a application on the website, and use the token shown in your application
            priority=0,                                                               # Read more about priority here: https://pushover.net/api#priority
            sound="vibrate",                                                         # A list of sounds can be found here: https://pushover.net/api#sounds
            events=[Events.DROP_CLAIM, Events.STREAMER_ONLINE],                          # Only these events will be sent
        )
    ),
...
twitch_miner.mine(
    [
        Streamer("tysead", settings=StreamerSettings()),
#        Streamer("basetradeTV", settings=StreamerSettings()),
#        Streamer("valair", settings=StreamerSettings()),
#        Streamer("yakov", settings=StreamerSettings()),
#        Streamer("zchum", settings=StreamerSettings()),
